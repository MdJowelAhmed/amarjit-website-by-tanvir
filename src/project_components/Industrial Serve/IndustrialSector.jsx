...
function IndustrialSector() {
	...
	return (
		<div className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl lg:py-24 mx-auto">
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{sectors.map((sector) => (
					<Card
						key={sector.id}
						className=" h-full hover:shadow-lg border-2 border-[#4f3075] transition-shadow duration-300 text-center">
						<CardHeader className="pb-2">
							<div className="flex flex-col items-center gap-4">
								...
								<div>
									<h3 className="text-2xl font-semibold text-gradient-heading">
										{sector.heading}
									</h3>
									{sector.subheading && (
										<p className="text-base font-semibold text-gradient-heading">
											{sector.subheading}
										</p>
									)}
								</div>
						...
						<CardContent>
							<p className="text-black">{sector.content}</p>
						</CardContent>
					</Card>
				))}
			</div>
