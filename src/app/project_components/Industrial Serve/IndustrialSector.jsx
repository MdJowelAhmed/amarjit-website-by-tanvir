    ...
    <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl lg:py-24 mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sectors.map((sector) => (
          <Card
            key={sector.id}
            className="get-in-touch-card h-full hover:shadow-lg transition-shadow duration-300 text-center"
          >
            <CardHeader className="pb-2">
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  ...
                  {sector.subheading && (
                    <p className="text-sm text-white">{sector.subheading}</p>
                  )}
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <p className="text-white">{sector.content}</p>
            </CardContent>
          </Card>
        ))}
      </div>
